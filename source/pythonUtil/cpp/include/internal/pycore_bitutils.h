...
_Py_bswap16(uint16_t word)
{
    ...
    Py_BUILD_---ERT(sizeof(word) == sizeof(unsigned short));
...
#elif defined(_MSC_VER)
    Py_BUILD_---ERT(sizeof(word) == sizeof(unsigned long));
...
#if SIZEOF_INT >= 4
    Py_BUILD_---ERT(sizeof(x) <= sizeof(unsigned int));
    return __builtin_popcount(x);
#else
    // The C standard guarantees that unsigned long will always be big enough
    // to hold a uint32_t value without losing information.
    Py_BUILD_---ERT(sizeof(x) <= sizeof(unsigned long));
...
_Py_bit_length(unsigned long x)
{
    ...
    Py_BUILD_---ERT(sizeof(unsigned long) <= 4);
    unsigned long msb;
